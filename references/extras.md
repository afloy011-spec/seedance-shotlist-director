...
Read this file only when triggered: the user asks for a vertical/square version (9:16, 1:1, Reels/Shorts/TikTok, «вертикальная версия») or provides a music track / BPM («монтаж под 120 BPM», attaches audio, names a song).
...
2. **On request** ("сделай вертикальную версию", "нужен 9:16"): re-render the board — or add a per-prompt variant block — with **recomposed framing**, not just a changed ratio word:
